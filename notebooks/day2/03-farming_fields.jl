...
begin
	...
	In this synthesis exercise, you will help Daisy manage her fields. The fields are of peculiar shape: they are all in the shape of *$(shape_type)*! These little fields are part of a big piece of land that extends from 0 to 100 in both the x- and y-direction.
	
	> **Note:** you can choose the flavour of this exercise by picking the shapes of the fields, `circles` and `rectangles` are quite easy, while `triangles` will test your geometry skills!
...
md"""
The problem is that daisy does not know how many seeds to buy. She is an experienced gardener, however, and only needs to know an estimation of the surface area that needs sowing.
...
A little over-preparation never hurt anyone. Before commiting to this crazy plan, it would be wise to check how the front garden will look like in blooming season.
...
> 
> Computing this might be a bit tricky. So Daisy proposes an alternative solution to estimate the area. She suggest randomly throwing a large number (say, 100,000) of seeds in the big field and count which fraction land in one of the shapes. This fraction is proportional to the part of the land covered by a field.
> 
...
pinc((xc, yc, R),) = ((x, y),) -> (x-xc)^2 + (y-yc)^2 < R^2

# ╔═╡ 8f7fb0c4-ba18-4025-b322-62c2d17aced8
pinc((x,y), (xc, yc, R)) = (x-xc)^2 + (y-yc)^2 < R^2
...
begin
	p = plot(aspect_ratio=:equal, xlim=(0,100), ylim=(0,100))
	θs = 0:0.01:2pi
	for (xc, yc, R) in circles
		xs, ys = R .* cos.(θs) .+ xc, R .* sin.(θs) .+ yc
		plot!(xs, ys, label="", fill=true)
	end
...
n_points_inshape = count(any(s->pinc(p,s), shapes) for p in points)

# ╔═╡ 95fecfbd-6969-498b-b909-ac945a4e9fbd
f_covered = n_points_inshape / n_points

# ╔═╡ b8faefbe-fb08-46cb-b349-5d2fd25cdf05
area_covered  = 100^2 * f_covered
