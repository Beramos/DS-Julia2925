...
# ╔═╡ a2f1a019-306a-46f4-88df-21658a8fddf6
using Plots, PlutoUI

# ╔═╡ df2745a5-8bc3-45db-bc1b-39b4d0e78b15
using DifferentialEquations, ModelingToolkit

# ╔═╡ 3d9f2e54-376c-42e0-97ba-288402969585
using ModelingToolkit: t_nounits as t, D_nounits as D

# ╔═╡ 072e5dca-0982-4803-b303-043bda8e017d
using Symbolics

# ╔═╡ 5ae524ed-6bad-4ffc-a2b5-8a18519c6460
md"""
# Mechanical systems using MTK

This is a brief tutorial on how to model mechanical systems via the Lagrangian using ModelingToolkit.jl. The Lagrangian is very powerful and flexible but requires quite a bit of calculus to obtain equations of motion, something that MTK can easily do for us by automating differentiation and system conversion. We will illustrate this on the double pendulum, a simple system that can exhibit pretty chaotic behavior.
"""

# ╔═╡ 6f2a9349-0497-4c97-94e6-4f01b0fa2ca1
md"""
## Lagrangian mechanics

In introductory Newtonian physics, one can derive equations of motion as differential equations of simple systems. Here, one picks a coordinate system, applies a force balance and writes the net forces on an ax via Newton's second law (for example, along the y-axis):
...
For more complex systems involving multiple interacting objects, constraints, or the use of non-Cartesian coordinate systems, directly applying Newton's laws can quickly become very challenging. Instead, [Lagrangian mechanics](https://en.wikipedia.org/wiki/Lagrangian_mechanics#Lagrangian) is much more general and scalable. This approach begins with the Lagrangian $L$, which is defined as
...
The equations of motion for the system can then be systematically derived using the Euler-Lagrange equations:

$$\frac{\mathrm{d}}{\mathrm{d}t}\left(\frac{\partial L}{\partial \dot{q}_j}\right) = \frac{\partial L}{\partial q_j}\,,$$

Here, $q_j$ represent the **generalized coordinates** — a set of independent variables that fully describe the configuration of the system. The term $\dot{q}_j$ refers to the time derivatives of these generalized coordinates, often called **generalized velocities**.

"""

# ╔═╡ 88556f6d-5c33-4426-8705-213374b077a4
md"""
## Single pendulum

A single pendulum is a good example to start with. We only need to keep track of one angle.

![A single pendulum.](https://i.ibb.co/MkRw1FZg/single-and-double-pendulum-excalidraw.png)
"""

# ╔═╡ f275bd58-f969-4059-a7ee-42c782941a25
@variables θ(t)

# ╔═╡ bc86a834-c23d-4ba3-83d1-c7addbfb308b
θ̇ = D(θ)  # angular velocity

# ╔═╡ 462a3b01-dee0-417b-b1ba-afd5bc542149
D_θ = Differential(θ)  # derivative to angule

# ╔═╡ 0019810a-1ad7-43d2-bd07-7309503186c2
D_θ̇ = Differential(θ̇)  # derivative to angular velocity

# ╔═╡ d0b72515-c7d6-4e4b-a71b-12b0787d842d
@parameters m l g

# ╔═╡ 2e2764ed-a2e7-4e4c-bd26-3ace878c366b
kin_pen = m * l^2 * θ̇^2 / 2  # kinetic energy

# ╔═╡ 3296c51c-f9e8-4336-8348-7a804e482b2f
md"Using the symbolic differentiation, we derive the Euler-Lagrange equations."
...
# ╔═╡ ef437022-5a80-447a-8cda-52a54b790885
x = l * sin(θ)
...
# ╔═╡ 7704040a-e5f9-4825-8c85-a9a5b898c1f8
Lp = kin_pen - pot_pen  # Lagrangian
...
# ╔═╡ de710a00-987d-497d-a635-524cbbd33043
md"Fase plane"

# ╔═╡ 13dbe96c-9c14-4700-b64a-e97b3c4ddf78
plot(sol_pen, idxs=(x,y), xlab="x", ylab="y")

# ╔═╡ ec7a8b04-6ea1-401b-a2e5-0e0b9a887e10
md"""
## Double pendulum

Nothing we could not do before! Let us now go to a double pendulum. Because the Cartesian coordinates are a bit more difficult to keep track of, let us combine both polar and Cartesian coordinates in the system. MTK will sort everything out for us.

![Double pendulum](https://i.ibb.co/VcK70fh1/double-pendulum-excalidraw.png)

"""

# ╔═╡ b3f8c879-0d68-48d2-a863-f107d307b421
@variables θ₁(t) θ₂(t) ω₁(t) ω₂(t) x₁(t) y₁(t) x₂(t) y₂(t)

# ╔═╡ 7328695a-8253-4dc1-a39d-9aa19fe8061d
l1, l2 = l, l  # keep lengths the same for simplicity

# ╔═╡ bd9c7b41-e860-4a52-af6c-654ceff03072
m1, m2 = m, m  # ditto

# ╔═╡ 964df5b3-1b76-45d1-b00d-7c822a912672
md"Linking Cartesian and polar coordinates..."

# ╔═╡ 231a6837-da19-4875-be6a-4fe7bff77c76
x1 = -l1 * sin(θ₁)

# ╔═╡ 877ef06c-c057-433f-b5db-6e8ffb26c245
y1 = -l1 * cos(θ₁)

# ╔═╡ 182d9159-0831-43b3-93f5-65dbbc679ba0
x1^2 + y1^2 |> simplify  # sanity check

# ╔═╡ d91fd53a-e832-4bc7-8b24-755ec0925728
x2 = x1 - l2 * sin(θ₂)

# ╔═╡ 12ec02ef-70e8-4294-997e-c4b3af08a46e
y2 = y1 - l2 * cos(θ₂)

# ╔═╡ 8a0752b2-8e15-4dc5-ab4e-008f9109b974
# kinetic energy

# ╔═╡ 73451918-f1e3-441b-9dfc-7c63958822b2
T1 = 1//2 * m1 * (D(x1)^2 + D(y1)^2)

# ╔═╡ 833af211-761f-437e-9b2e-11fcb1eea2a0
T2 = 1//2 * m2 * (D(x2)^2 + D(y2)^2)

# ╔═╡ e9a6542d-66fe-4683-a32e-73b92f250b22
kin_dp = T1 + T2 |> expand_derivatives |> simplify

# ╔═╡ 9cfcd87f-70c3-49ab-8843-9c7eef028c09
# potential energy

# ╔═╡ 0938165f-b259-442a-935f-919c08344194
pot_dp = m1 * g * y1 + m2 * g * y2

# ╔═╡ 2f2d0341-7a44-4c39-b79d-d8b238086702
Ldp = kin_dp - pot_dp  # Lagrangian

# ╔═╡ 42c62f25-393f-4329-b82b-83f61c2eb51b
# now compute the Euler-Langrange equations

# ╔═╡ 00143e88-5691-4330-a8fb-9b60360cc187
Dω₂ = Differential(ω₂)

# ╔═╡ ebec2834-b407-4b3b-ad36-f5b77cbdeba2
Dθ1 = Differential(θ₁)

# ╔═╡ 8fd802c1-525e-43ec-8629-c2cc6e0fb02d
Dθ̇1 = Differential(D(θ₁))

# ╔═╡ 7179d5e5-2518-49c2-a666-d0be8e6b5e88
Dθ2 = Differential(θ₂)

# ╔═╡ 56f73f33-05a1-4a66-b3cb-22729281211e
Dθ̇2 = Differential(D(θ₂))

# ╔═╡ dcd48db5-074c-479b-a188-6977b1d3b813
# obtain the equations
sys_dp = [expand_derivatives(D(Dθ̇1(Ldp)) - Dθ1(Ldp)) ~ 0, expand_derivatives(D(Dθ̇2(Ldp)) - Dθ2(Ldp)) ~ 0]

# ╔═╡ b0bad699-50cb-4980-984c-dda6d71956cc
# solve for D^2(θ₁) and D^2(θ₂)
E1, E2 = symbolic_linear_solve(sys_dp, [D(D(θ₁)), D(D(θ₂))]) .|> simplify

# ╔═╡ 35ed7ec7-ffb3-410b-a2f6-cd3435f99bb0
# build the system
@mtkbuild double_pend = ODESystem([D(D(θ₁))~E1,
                                 D(D(θ₂))~E2,
								x₁ ~ x1, y₁ ~ y1, x₂ ~ x2, y₂ ~ y2], t)

# ╔═╡ 4ae87b4b-8e7a-443f-82d6-bba04a671d1f
md"Would not like to have done that by hand... Note that we added the definitions of the Cartesian coordinates, which were promptly eliminated as they can be computed on the fly."

# ╔═╡ 99e198bc-1062-4b40-8147-06ab3930136f
md"Nice animation for seeing this live."

# ╔═╡ 37f68463-a24e-472c-bff6-1a4a5fc579da
tsteps = 0:0.1:25

# ╔═╡ 108c3ed9-faf9-43c5-8049-acc0fd1f909b
@bind ti PlutoUI.Clock(max_value=length(tsteps), interval=0.2, repeat=true)

# ╔═╡ a1b0a339-58e7-47cb-b2de-3c5e28ae75c8
@bind θ1_0 Slider(-3:0.1:3, default=1.6, show_value=true)

# ╔═╡ 1f242d5f-31b9-4e85-b062-cb311ce3c4ad
@bind θ2_0 Slider(-3:0.1:3, default=-1.3, show_value=true)

# ╔═╡ acf023a1-82ce-4b60-a45c-c0202be429c8
@bind θ1′_0 Slider(-3:0.1:3, default=1.5, show_value=true)

# ╔═╡ ee659eaa-47fe-41de-aae4-9952daf7cb80
@bind θ2′_0 Slider(-3:0.1:3, default=-1, show_value=true)

# ╔═╡ 869834fa-fc93-48a1-ab47-d023af200312
pend_dp = ODEProblem(double_pend, [θ₁=>θ1_0, D(θ₁)=>θ1′_0, θ₂=>θ2_0, D(θ₂)=>θ2′_0], extrema(tsteps), [g=>9.81, l=>0.5, m=>.1])

# ╔═╡ 57852823-1a4d-4364-97da-bf4ea8c60249
dp_sol = solve(pend_dp)

# ╔═╡ c70bcfa6-76b8-439d-8b87-1f0a4f389842
# plotting the angles as a function of time
plot(dp_sol, idxs=[θ₁, θ₂])

# ╔═╡ a48d050a-52e5-412a-ba3b-acf54e1e3876
# phase plots of the Cartesian coordinates
begin
	plot(dp_sol, idxs = (x₁, y₁))
	plot!(dp_sol, idxs = (x₂, y₂))
end

# ╔═╡ 93615b6f-acfd-49f8-b868-64ae95cac534
let
	tval = tsteps[ti]
	p = plot(legend=false, xlims=[-1.1, 1.1], aspect_ratio=:equal)
	xlabel!("x")
	ylabel!("y")
	x1, x2 = dp_sol(tval; idxs=[x₁, x₂])
	y1, y2 = dp_sol(tval; idxs=[y₁, y₂])
	
	plot!([0, x1, x2], [0, y1, y2], m=:o, lw=2)
	plot!(dp_sol, idxs = (x₁, y₁), alpha=0.5)
	plot!(dp_sol, idxs = (x₂, y₂), alpha=0.5)
end

# ╔═╡ 4f7b2b9e-93cf-4b25-a5e9-56a4f37b904f
md"""
## Planetary motion

For fun, we can also simulate a simple two-body system. I added the parameters and initial conditions of the Sun-Earth system. The sky is literaly the limit.
"""

# ╔═╡ e885950d-0587-4586-b559-01568648e402
@variables r(t)

# ╔═╡ 0f53b523-1240-41be-b862-9de03a5dcd33
@parameters M₁ M₂ G

# ╔═╡ dd7bf9c4-6deb-4e88-8475-fa1b6f30f855
kin_plan = M₁ / 2 *(D(x₁)^2 + D(y₁)^2) + M₂ / 2 *(D(x₂)^2 + D(y₂)^2)

# ╔═╡ 54b930be-4ddf-4b72-8299-8bf52d5a23e2
dist = √((x₁ - x₂)^2 + (y₁ - y₂)^2)

# ╔═╡ fbd3b9d1-a35c-478e-8ee9-ffe5418f1ea3
pot_plan = - G * M₁ * M₂ / dist

# ╔═╡ e44dff15-6425-4fee-bb4f-47c94f6e2dfa
L_plan = kin_plan - pot_plan

# ╔═╡ 64863b90-bef7-47b4-8661-233c135e01d3
begin
	Dx1 = Differential(x₁)
	Dx2 = Differential(x₂)
	Dy1 = Differential(y₁)
	...
	Dẋ1 = Differential(D(x₁))
	Dẋ2 = Differential(D(x₂))
	Dẏ1 = Differential(D(y₁))
	Dẏ2 = Differential(D(y₂))
	
end

# ╔═╡ 9829811a-8e44-4864-81b6-571d41c0a15d
symbolic_linear_solve(expand_derivatives(D(Dẋ1(L_plan))) -expand_derivatives((Dx1(L_plan)))~0, D(D(x₁)))

# ╔═╡ e3966f91-c673-4771-ab46-9744c0b6bc93
expand_derivatives((Dx1(L_plan)))

# ╔═╡ 44855a7f-0c53-49ad-9d02-5b5a91bc145b
sys_plan = [expand_derivatives(D(Dv̇(L_plan)) - Dv(L_plan)) ~ 0 for (Dv, Dv̇) in zip([Dx1, Dx2, Dy1, Dy2], [Dẋ1, Dẋ2, Dẏ1, Dẏ2])] .|> simplify

# ╔═╡ e7347b64-0174-4196-87da-4611cfc9295b
Ex1, Ex2, Ey1, Ey2 = symbolic_linear_solve(sys_plan, [D(D(x₁)), D(D(x₂)), D(D(y₁)), D(D(y₂))])

# ╔═╡ 74388751-867d-4a9b-9052-5e7aee3bb5dc
@mtkbuild planets = ODESystem([D(D(x₁))~Ex1, D(D(y₁))~Ey1, D(D(x₂))~Ex2, D(D(y₂))~Ey2, dist ~ r], t)

# ╔═╡ 61286748-fd6b-4e02-80c2-279a8df6247b
M = 1e1

# ╔═╡ c1698ee3-68cb-4546-959f-c1517816c4fe
plan_prob = ODEProblem(planets, [x₁=>0, y₁=>0, D(x₁)=>0, D(y₁)=>0., x₂=>150.96e9, y₂=>0, D(x₂)=>0, D(y₂)=>29.78e3], (0.0, 365*24*3600*10), [M₁=>1.9891e30, M₂=>5.972e24, G=>6.67e-11]);

# ╔═╡ 0dd20f4c-d47c-40ff-8452-513cb1fa2c3b
plan_sol = solve(plan_prob, reltol=1e-9, saveat=24*3600);

# ╔═╡ 3f6e4dd3-c60f-4668-aa34-f782fa649557
size(plan_sol)

# ╔═╡ eebf4232-d7c3-4552-b956-b7124d961e24
begin
	plot(plan_sol, idxs=(x₁, y₁), aspect_ratio=:equal)
	plot!(plan_sol, idxs=(x₂, y₂))
end

# ╔═╡ 39e92671-37e4-4847-a4ef-c4516b7289f1
plot(plan_sol, idxs=r)

# ╔═╡ 6bef834b-a2f2-4c08-855d-e11ae620c86f
TableOfContents()
...
manifest_format = "2.0"
project_hash = "c35578ab01b0f48901656a4ea49e0586a259fb17"
