...
Luxor.jl draws 2D vector graphics from a sequence of simple, readable commands — `circle` and `poly` to draw shapes, `sethue` to pick a colour (it sets the current "hue"), `text` to add lettering — and saves the result as PNG or SVG. In this adventure you design a small logo from primitives — a gentle, visual way into the language that ends with something you can actually share.
...
1. Work through the [quickstart "design a logo" tutorial](https://juliagraphics.github.io/LuxorManual/stable/tutorial/quickstart/) until you can produce and save your own drawing.
2. Study how the real three-disc Julia logo is built from primitives on the [examples page](https://juliagraphics.github.io/LuxorManual/stable/example/examples/).
...
## 4. Resources
- Luxor.jl documentation ([source](https://juliagraphics.github.io/LuxorManual/stable/))
...
You call `optimize(f, x0, method)` with a method such as `NelderMead()` (gradient-free) or `BFGS()` (gradient-based), and Optim reports the minimiser, the minimum value, and diagnostics like the number of iterations. To compare these methods you will use three classic two-dimensional test functions:
 
- the **Rosenbrock** function — a long, narrow, curved valley that is easy to define but awkward to descend;
- **Himmelblau's** function — which has *four* equally-good global minima, so the starting point decides which one you reach;
- **Beale's** function — another sharp-valley trap that punishes naive steps.
Seeing how each algorithm copes with these very different landscapes is the whole point of the exercise.
...
2. Minimise **Himmelblau's function** from several starting points and tabulate which of the four minima each one converges to.
3. Minimise **Beale's function** as a second stress test. Formulas for both are on the [test-functions page](https://en.wikipedia.org/wiki/Test_functions_for_optimization).
...
- Dessert-problem code and notebook ([source](https://github.com/MichielStock/Teaching/tree/master/Optimal_transport))
...
> **Note:** if this grabs you, project **04 (Modelling a zombie outbreak)** is the full-size agent-based-modelling project — this adventure is the bite-sized taster.
...
Graphs.jl gives you one consistent way to build a network and then interrogate its structure — how many connections each node has, which nodes sit at the centre of things, the shortest route between two points — regardless of where the network came from. GraphMakie turns the result into a picture. Starting on a small, familiar graph lets you learn the workflow before pointing it at real data.
 
## 3. Assignments
 
1. Learn the workflow on the built-in Zachary karate club graph (`smallgraph(:karate)`) — a classic 34-member friendship network recorded in a 1970s university karate club that later split into two factions, and the de-facto "hello world" of network science. Compute `degree` and `betweenness_centrality` using the [centrality page](https://juliagraphs.org/Graphs.jl/stable/algorithms/centrality/) and plot it with the [plotting guide](https://juliagraphs.org/Graphs.jl/stable/first_steps/plotting/).
...
An image is just an array of coloured numbers, so filtering it is array arithmetic. This adventure runs a couple of standard filters and then builds an edge detector — and connects straight back to the day-1 [images-and-cellular-automata notebook](https://github.com/Beramos/DS-Julia2925/blob/master/notebooks/day1/03-exercises.jl).
